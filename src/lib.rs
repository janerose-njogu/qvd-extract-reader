...
#[pymodule]
fn qvd(_py: Python, module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add_function(wrap_pyfunction!(read_qvd_data, module)?)?;
    Ok(())
}

#[pyfunction]
fn read_qvd_data(py: Python, file_name: String) -> PyResult<Py<PyDict>> {
    ...
    if let Ok(f) = File::open(&file_name) {
        ...
        let buf: Vec<u8> = read_qvd_data_to_buffer(f, binary_section_offset);
        ...
        for field in qvd_structure.fields.headers {
            ...
            let column_values: Vec<Option<String>> =
...
fn read_qvd_data_to_buffer(mut f: File, binary_section_offset: usize) -> Vec<u8> {
    ...
    while i < end {
        ...
        match byte {
            0 => {
                ...
                let utf8_bytes: Vec<u8> = buf[string_start..i].to_vec().to_owned();
...
fn get_row_indexes(buf: &[u8], field: &QvdFieldHeader, record_byte_size: usize) -> Vec<i64> {
    ...
    for chunk in chunks {
        ...
        let end: usize = bits.len() - field.bit_offset - field.bit_width;
...
fn get_xml_data(file_name: &str) -> Result<String, io::Error> {
    match read_xml_section(file_name) {
        Ok(mut reader) => {
...
fn read_xml_section<P>(filename: P) -> io::Result<io::BufReader<File>>
where
    P: AsRef<Path>,
{
...
mod tests {
    ...
    fn test_double() {
        ...
        let field: QvdFieldHeader = QvdFieldHeader {
            length: buf.len(),
            offset: 0,
        ...
        let res: Vec<Option<String>> = get_symbols_as_strings(&buf, &field);
    ...
    fn test_int() {
        ...
        let res: Vec<Option<String>> = get_symbols_as_strings(&buf, &field);
        let expected: Vec<Option<String>> = vec![Some(10.0.to_string()), Some(20.0.to_string())];
        assert_eq!(expected, res);
    }
    ...
    fn test_string() {
        ...
        let res: Vec<Option<String>> = get_symbols_as_strings(&buf, &field);
        let expected: Vec<Option<String>> = vec![Some("example text".into()), Some("rust".into())];
        assert_eq!(expected, res);
    }
    ...
    fn test_utf8_string() {
        ...
        let res: Vec<Option<String>> = get_symbols_as_strings(&buf, &field);
        let expected: Vec<Option<String>> = vec![Some("也有中文简体字".into()), Some("🐍🦀".into())];
        assert_eq!(expected, res);
    }
    ...
    fn test_bitslice_to_vec() {
        ...
        let bits: &BitSlice<Msb0, u8> = BitSlice::<Msb0, _>::from_slice(&mut x[..]).unwrap();
        let target: &BitSlice<Msb0, u8> = &bits[27..32];
        let binary_vec: Vec<u8> = bitslice_to_vec(&target);
    ...
    fn test_get_row_indexes() {
        ...
        let record_byte_size: usize = buf.len();
        let res: Vec<i64> = get_row_indexes(&buf, &field, record_byte_size);
